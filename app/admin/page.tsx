...
function AdminContent() {
  ...
  const [formulas, setFormulas] = useState<Array<{ id: string; name: string; price: number | null; image_url?: string | null }>>([]);
  ...
  const fetchFastEntryResources = async () => {
    ...
    if (!dishesError) {
      ...
      const formulasResult = currentRestaurantId
        ? await supabase.from("restaurant_formulas").select("*").eq("restaurant_id", currentRestaurantId).eq("active", true)
        : await supabase.from("restaurant_formulas").select("*").eq("active", true);
      ...
      if (formulasResult.error) {
        ...
        setFormulas(
          (formulasResult.data || []).map((row) => ({
            id: String((row as any).id || ""),
            name: String((row as any).name || ""),
            price: parsePriceNumber((row as any).price),
