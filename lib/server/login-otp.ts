...
function resolveOtpMailerConfig() {
  const user = String(process.env.OTP_EMAIL_USER || process.env.EMAIL_USER || process.env.GMAIL_USER || "").trim();
  const pass = String(
    process.env.OTP_EMAIL_PASSWORD || process.env.EMAIL_APP_PASSWORD || process.env.GMAIL_APP_PASSWORD || ""
  ).trim();
  if (!user || !pass) {
    throw new Error("SMTP OTP non configuré. Définissez OTP_EMAIL_USER/OTP_EMAIL_PASSWORD ou EMAIL_USER/EMAIL_APP_PASSWORD.");
  }
  return { user, pass };
}
...
export async function sendDashboardOtpEmail(params: {
  ...
  await transporter.sendMail({
    from: user,
    to,
    subject: `Code de vérification ${scopeLabel}`,
    text: `Votre code de vérification ${scopeLabel} est ${code}. Il expire dans 10 minutes.`,
    html: `
      <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#111">
        <h1 style="font-size:22px;margin:0 0 16px">Vérification ${scopeLabel}</h1>
        <p style="margin:0 0 12px">Votre code de vérification est :</p>
